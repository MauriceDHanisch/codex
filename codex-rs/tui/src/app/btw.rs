...
const BTW_PROMPT_MAX_BYTES: usize = 6 * 1024;
const BTW_QUESTION_MAX_BYTES: usize = 2 * 1024;

const BTW_PROMPT_PREFIX: &str = concat!(
    "Answer the user's side question using only the reference conversation below. ",
...
impl App {
    ...
    async fn btw_conversation(&self, thread_id: ThreadId) -> String {
        ...
        super::thread_title::recent_conversation_messages(items.iter()).unwrap_or_default()
    }
...
fn btw_prompt(conversation: &str, question: &str) -> String {
    ...
    let conversation = truncate_to_bytes(conversation, conversation_budget);
