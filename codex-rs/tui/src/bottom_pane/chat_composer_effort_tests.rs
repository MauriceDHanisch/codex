...
fn effort_composer_baseline_repeat_and_lowering_do_not_replay() {
    ...
    composer.set_status_line(Some(vec![Line::from("gpt-5.4 high · main")]));
...
fn effort_transition_never_replaces_a_footer_flash() {
    ...
    composer.set_status_line(Some(vec![Line::from("gpt-5.4 ultra · main")]));
    composer
        .footer
...
fn effort_transition_keeps_the_full_footer_row() {
    ...
    composer.set_status_line(Some(vec![Line::from("gpt-5.4 high · feature-branch")]));
    composer.set_active_reasoning_effort(
        Some(&ReasoningEffort::High),
        /*animations_enabled*/ true,
    );
